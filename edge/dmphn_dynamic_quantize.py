...
# ---------------------------
# Create and load model components
...
class DMPHN(nn.Module):
    def __init__(self, encoders, decoders):
        ...
        # important for NNCF to find and manipulate layers 
        # (e.g. pruning); regular attributes wouldn't be picked up.
...
try:
    ...
    nncf_config_dict = {
        ...
        "compression": {
            ...
            "params": {
                "pruning_flops_target": 0.5,  # reduce FLOPs by 50% (A test to check effects)
...
# --------------------------------------------------------
# Static quantization fails on Windows due to 'quantized::conv2d.new'
# error, especially when ConvTranspose layers are present.
# So, this section is disabled.

# --------------------------------------------------------
# Dynamic Quantization (works on Windows)
# --------------------------------------------------------
try:
    print("\n--- Dynamic Quantization ---")
    from torch.quantization import quantize_dynamic

    # Only quantize nn.Linear layers (safe choice).
    # Conv layers stay in float => avoids 'quantized::conv2d' crash.
    model_dynamic = quantize_dynamic(
        model,  # we can quantize the baseline or pruned model here.
        {nn.Linear},
        dtype=torch.qint8
    )

    dyn_inference_time = measure_inference_time(model_dynamic, iterations=5)
    dyn_file_size = save_model_and_get_size(model_dynamic, "dyn_quant_model.pth")
    dyn_dataset_psnr, _ = process_images(model_dynamic, SAMPLE_DIR, GT_DIR)

    print("\nDynamic quantized model results:")
    print("  PSNR (dataset):", dyn_dataset_psnr)
    print("  Average inference time:", dyn_inference_time, "seconds per input")
    print("  Model file size:", dyn_file_size, "bytes")
